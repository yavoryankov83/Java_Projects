...
//     const $VERB = 'GET';
//     const $DATATYPE = 'json';
//     const $userPostsDiv = $('#userPosts-container');
//     countPostByUserPage++;
//
//     $.ajax({
//         type: $VERB,
...
//
//                 let $postData = $(`<div class="post-content">
// <!--Post Date-->
// <div class="post-date hidden-xs hidden-sm">
// <h5>${post.user.username}</h5>
// <p class="text-grey">Sometimes ago</p>
// </div>
// <img src="${post.postPhoto}" alt="post-image" class="img-responsive post-image"/>
// <div class="post-container">
// <img src="${post.user.userPhoto}" alt="user" class="profile-photo-md pull-left"/>
// <div class="post-detail">
// <div class="user-info">
// <h5><a href="/timeline-about.html/${post.user.id}" class="profile-link">${post.user.firstName + ' ' + post.user.lastName + ' (' + post.user.username + ')'}</a>
// <p class="text-muted">Published a post on ${$datetime}</p>
// </div>`);
//
//                 let $likes = getPostLikes(postId);
//
//                 let $postLikes = $(`<div class="reaction">
// <a id="lk" class="btn text-green"><i class="icon ion-thumbsup"></i> ${$likes}</a></div>`);
//
//                 let $lineDivider = (`<div class="line-divider"></div>`);
//
//                 let $postText = $(`<div class="post-text">
// <p>${post.content}<iclass="em em-anguished"></i> <i class="em em-anguished"></i> <iclass="em em-anguished"></i></p>
// </div>`);
//
//                 $userPostsDiv.append($postData).append($postLikes).append($lineDivider).append($postText).append($lineDivider);
//
//                 let $comments = getCommentData(postId);
//                 $comments.forEach((comment) => {
//                     let $postCommentData = (`<div class="post-comment">
// <img src="${comment.user.userPhoto}" alt="" class="profile-photo-sm"/>
// <p><a href="/timeline-about.html/${comment.user.id}" class="profile-link">${comment.user.username} </a><i
// class="em em-laughing"></i> ${comment.content} </p>
// </div>`);
//                     $userPostsDiv.append($postCommentData);
//                 });
//
//                 let $makeComment = (`<div class="post-comment">
// <img src="${post.user.userPhoto}" alt="" class="profile-photo-sm"/>
// <input type="text" class="form-control" placeholder="Post a comment">
// </div>`);
//                     $userPostsDiv.append($makeComment);
//             });
//         },
//         error: function () {
...
<p class="text-muted" style="font-size: small">Published a post on ${$datetime} ${$postLocation}</p>
                ...
                let $reaction = (`<div class="reaction">
                      ...
                      <a id="${postId}" class="btn text-red" onclick="deletePosts(${postId})"><ion-icon name="trash" size="smaller"></ion-icon></a>
                    </div>`);
                ...
                if (principalUsername !== post.user.username){
                    $(`#${postId}`).hide();
                    $(`#${'post-update' + postId}`).hide();
                }
...
<a id="${'comment-likes' + commentId}" class="btn text-green" style="font-size: medium" onclick="createLikeToComment(${commentId})"><ion-icon name="heart" size="smaller"></ion-icon> ${getCommentLikes(comment.id)}</a>
                      ...
                      <a id="${commentId}" class="btn text-red" onclick="deleteComments(${commentId})"><ion-icon name="trash" size="smaller"></ion-icon></a>
                    </div>`);

                    $commentDiv.append($postComment).append($reaction).append($lineSeparator);
                    // $postDetail.append($commentDiv);

                    if (principalUsername !== comment.user.username){
                        $(`#${commentId}`).hide();
...
<p id="${'child-comment-content' + childCommentId}" style="font-size: small" class="text-green">${childComment.content}</p>
                      ...
                      <a id="${childCommentId}" class="btn text-red" onclick="deleteChildComments(${childCommentId})"><ion-icon name="trash" size="smaller"></ion-icon></a>
                      <a id="${'child-comment-likes' + childCommentId}" class="btn text-green" style="font-size: medium" onclick="createLikeToChildComment(${childCommentId})"><ion-icon name="heart" size="smaller"></ion-icon> ${getCommentLikes(childComment.id)}</a>
                        ...
                        if (principalUsername !== childComment.user.username){
                            $(`#${childCommentId}`).hide();
                            $(`#${'child-comment-update' + childCommentId}`).hide();
                        }
                    });
...
//FUNCTION TO UPDATE POST DYNAMICALLY
function updatePostAll(postId) {
    let $contentToUpdate = document.getElementById(`${'post-content' + postId}`).textContent.trim();
    let $input = $(`<div id="${'post-input' + postId}"><input contenteditable="true" type="text" class="form-control" required minlength="3" maxlength="255" value="${$contentToUpdate}" autofocus>
                    <input contenteditable="true" type="text" class="form-control" required minlength="3" maxlength="255" value="location" autofocus>
</div>`);
    $(`#${'post-content' + postId}`).replaceWith($input);
    $(`#${'post-input' + postId}`).on('keypress',function(event) {
        if(event.key === "Enter") {
...
function updateParentComment(parentCommentId) {
    let $contentToUpdate = document.getElementById(`${'comment-content' + parentCommentId}`).textContent;
    $(`#${'comment-content' + parentCommentId}`).replaceWith(`<input contenteditable="true" id="${'comment-input' + parentCommentId}" type="text" autofocus class="form-control" required minlength="3" maxlength="255" value="${$contentToUpdate}">`);
...
//FUNCTION TO UPDATE CHILD COMMENT
function updateChildComment(childCommentId) {
    let $contentToUpdate = document.getElementById(`${'child-comment-content' + childCommentId}`).textContent;
    $(`#${'child-comment-content' + childCommentId}`).replaceWith(`<input contenteditable="true" id="${'child-comment-input' + childCommentId}" type="text" autofocus class="form-control" required minlength="3" maxlength="255" value="${$contentToUpdate}">`);
...
// function updateParentComment34(parentCommentId) {
//     $(`#${parentCommentId + 40000}`).replaceWith(`<input contenteditable="true" id="${parentCommentId + 1000}" type="text" class="form-control" placeholder="Update a comment">`)
//     $(`#${parentCommentId + 1000}`).on('keypress',function(event) {
//         if(event.key === "Enter") {
//             const $URL = `http://localhost:8080/api/v1/comments/${parentCommentId}`;
//             const $VERB = 'PUT';
//             const $DATATYPE = 'json';
//             let postContent = document.getElementById(`${parentCommentId + 1000}`).value;
//
//             let $data =
//                 {
//                     "content": postContent,
...
// }
//
// function updateParentCommentNova(parentCommentId) {
//     $(`#${'content' + parentCommentId}`).replaceWith(`<input contenteditable="true" id="${'input' + parentCommentId}" autofocus type="text" class="form-control" placeholder="Update a comment">`);
//     $(`#${'input' + parentCommentId}`).on('keypress',function(event) {
//         if(event.key === "Enter") {
//             const $URL = `http://localhost:8080/api/v1/comments/${parentCommentId}`;
//             const $VERB = 'PUT';
//             const $DATATYPE = 'json';
//             let postContent = document.getElementById(`${'input' + parentCommentId}`).value;
//
//             let $data =
//                 {
//                     "content": postContent,
...
// }
//
// function update(parentCommentId) {
//     $(`#${'update' + parentCommentId}`).on('keypress',function(event) {
//         if(event.key === "Enter") {
//             const $URL = `http://localhost:8080/api/v1/comments/${parentCommentId}`;
//             const $VERB = 'PUT';
//             const $DATATYPE = 'json';
//             let postContent = document.getElementById(`${'update' + parentCommentId}`).value;
//
//             let $data =
//                 {
//                     "content": postContent,
...
// }
//
// function updateParentCommentThymeleaf(parentCommentId) {
//     $(`#${'update' + parentCommentId}`).show();
//     update(parentCommentId);
// }
//
// function updateChildComment1(childCommentId) {
//     $(`#${childCommentId + 2000}`).replaceWith(`<input contenteditable="true" id="${childCommentId + 1000}" type="text" class="form-control" placeholder="Update a comment">`)
//     $(`#${childCommentId + 1000}`).on('keypress',function(event) {
//         if(event.key === "Enter") {
//             const $URL = `http://localhost:8080/api/v1/comments/${childCommentId}`;
//             const $VERB = 'PUT';
//             const $DATATYPE = 'json';
//             let postContent = document.getElementById(`${childCommentId + 1000}`).value;
//
//             let $data =
//                 {
//                     "content": postContent,
...
function createParentComment(userId, postId) {
    const $URL = `http://localhost:8080/api/v1/comments`;
    const $VERB = 'POST';
    const $DATATYPE = 'json';
    // let postContent = ('#postId').val();
    let postContent = document.getElementById(`${postId + 1000}`).value;
...
function createChildComment(userId, postId, commentId) {
    const $URL = `http://localhost:8080/api/v1/comments`;
    const $VERB = 'POST';
    const $DATATYPE = 'json';
    // let postContent = ('#postId').val();
    let postContent = document.getElementById(`${postId + 100}`).value;
...
//FUNCTION TO CREATE COMMENT LIKE ON BUTTON CLICK
function LikedCommentCreate(commentId) {
    const $URL = `http://localhost:8080/api/v1/likedComments/${commentId}`;
    const $VERB = 'POST';
    const $DATATYPE = 'json';
...
function loadContentModal(commentId, commentContent){
    var options = {
        modal: true
    };
    let $modalComment = (`<div id="modal4e" class="modal fade" id="updateComment" role="dialog" aria-labelledby="exampleModalLabel3" aria-hidden="true" >
<div class="modal-dialog" role="document" >
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="exampleModalLabel3">Update comment</h5>
                                                    ...
                                                    <div class="modal-body" >
                                                        ...
                                                        <div class="col-7">
                                                            <label for="update-comment-content">Update comment</label>
                                                            <input type="text" id="update-comment-content"
                                                                   class="form-control" name="update_content"
                                                                   form="update-comment" value="${commentContent}"
                                                    ...
                                                    <div class="modal-footer">
                                                        ...
                                                        <button type="button" class="btn btn-secondary" id="comment-content"
                                                                data-dismiss="modal" onclick="updateCommentData(${commentId})">Update comment
                                                        </button>
                                                    </div>
                                                </div>
                                            </div>
    ...
    //
    // // $('#updateComment').show();
    // // $('#updateComment').load('get-dynamic-content.php?modal='+modal).dialog(options).dialog('open');
}
...
//FUNCTION TO CREATE POST LIKE ON BUTTON CLICK
// function LikedPostCreate(myPostId) {
//     const $URL = `http://localhost:8080/api/v1/likedPosts/posts/${myPostId}`;
//     const $VERB = 'POST';
//     const $DATATYPE = 'json';
//
//     $.ajax({
...
//FUNCTION TO CREATE COMMENT TO POST WHEN CLICK "CREATE COMMENT"
// function createCommentToPost(userId, postId){
//     const $URL = `http://localhost:8080/api/v1/comments/users/${userId}/posts/${postId}/create`;
//     const $VERB = 'POST';
//     const $DATATYPE = 'json';
//
//     $.ajax({
...
//         },
//         error: function () {
//             alert('Error create comment to post!');
//         }
//     });
// }

// $('#createPost').keypress(function (k) {
//     let key = k.which;
//     if (key == 13){
//         createCommentToPost();
//     }
